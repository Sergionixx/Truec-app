    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div
          ...
          title={isOnline ? "Conectado al servidor backend Node.js (puerto 3001)" : "Operando en modo autónomo local (resiliente)"}
          aria-label={`Estado de conexión: ${isOnline ? "Servidor conectado" : "Modo local autónomo"}`}
        >
          <span className={`w-2 h-2 rounded-full ${isOnline ? "bg-emerald-400 animate-pulse" : "bg-amber-400"}`} />
          <span>{isOnline ? "API Online" : "Modo Local"}</span>
        </div>
      </div>
